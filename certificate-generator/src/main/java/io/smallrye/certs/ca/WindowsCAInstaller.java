...
public class WindowsCAInstaller {
    ...
    public static void installCAOnWindows(String cn, File ca) throws Exception {
        LOGGER.log(System.Logger.Level.INFO,
                "🔥 Installing CA certificate (issuer: {0}) into your operating system keychain. Make sure your are in a privileged (`run with administrator`) terminal.",
                cn);
        ...
        if (res != 0) {
            LOGGER.log(System.Logger.Level.ERROR,
                    "❌ Unable to install the CA certificate into the keychain. Please run: `certutil -addstore -v -f -user Root "
                            + ca.getAbsolutePath() + "` in a privileged terminal.");
            return;
        }
